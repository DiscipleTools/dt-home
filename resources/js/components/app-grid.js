...
class AppGrid extends LitElement {
  ...
  render() {
    return html`
      <div class="app-grid">
        ${this.appData
          .filter((app) => (!this.isSortableDragging ? !app.is_hidden : true))
          .map(
            (app, index) => html`
              <div
                ...
                this.appDataCopy[
                  this.appDataCopy.findIndex((item) => item.slug === app.slug)
                ].is_hidden
                  ? "none"
                  : "block"}"
                data-index="${index}"
                ...
                @click="${(event) =>
                  !this.editing && this.handleClick(event, app.slug, app)}"
                @mousedown="${(event) =>
                  this.handleMouseDown(event, index, app)}"
                draggable="${this.editing ? "true" : "false"}"
                @dragstart="${!this.isSortableDragging
                  ? (event) => this.handleDragStart(event, index)
                  : null}"
                @dragend="${!this.isSortableDragging
                  ? (event) => this.handleDragEnd(event, index, app)
                  : null}"
                @dragover="${!this.isSortableDragging
                  ? (event) => this.handleDragOver(event, index, app)
                  : null}"
                @dragleave="${!this.isSortableDragging
                  ? (event) => this.handleDragLeave(event, index, app)
                  : null}"
                @drop="${!this.isSortableDragging
                  ? (event) => this.handleDrop(event, index, app)
                  : null}"
              >
                ${this.editing
                  ? html`
                      <sp-action-menu
                        style="
                                        ...
                                        --mod-actionbutton-edge-to-text:10px !important;
                                            "
                        class="app-grid__remove-icon ${this.showRemoveIconId
                          ? ""
                          : "hidden"}"
                      ...
                      >
                        <sp-menu-item
                          @click="${(event) =>
                            this.toggleModal(event, index, app)}"
                          @touchstart="${(event) =>
                            this.toggleModal(event, index, app)}"
                          class="edit-menu"
                        >
                          <span>
                            ...
                            ${translate("edit_menu_label")}</span
                          >
                        </sp-menu-item>
                        <sp-menu-item
                          @click="${(event) =>
