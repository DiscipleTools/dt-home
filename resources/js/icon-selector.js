jQuery(document).ready(function ($) {
  ...
  $(".change-icon-button-selector").click(function (e) {
    ...
    let icon_input = $("input[name='" + $(e.currentTarget).data("icon") + "']");
  ...
  $(".dt-admin-modal-box").on("click", ".ui-button", function (e) {
    ...
    let icon_input = $("input[name='" + $(button).data("icon") + "']");
      ...
      $("#dialog_icon_selector_icons_search_spinner")
        ...
        .fadeIn("fast", function () {
          ...
          let filtered_icons = window.lodash.filter(icons, function (icon) {
            return (
              icon["class"] &&
              window.lodash.includes(icon["class"].toUpperCase(), query)
            );
          });
  ...
  function handle_icon_save(
    ...
    if ($(selected_icon).length) {
      const icon_selected_placeholder = $(
        `#${$(icon_input).data("icon_selected_placeholder")}`,
      );
