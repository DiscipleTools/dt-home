describe('RT054: Confirm specified user roles partner with no permission is unable to access coded app', () => {
    // Define shared data.
    const shared_data = {
    ...
    // The administrator navigates to the app settings page, selects a coded app, and verifies that the partner role is checked.
    it('The administrator navigates to the app settings page, selects a coded app, and verifies that the Partner role is checked.', () => {
        cy.session(
            'the_administrator_navigates_to_the_app_settings_page_selects_a_coded_app_and_verifies_that_the_partner_role_is_checked',
            () => {
                ...
                // Administrator confirms Partner role is checked and updates settings.
                cy.get(
                    'input.apps-user-role[name="roles[]"][value="partner"]'
                ).check()
    ...
    it('Administrator navigates to D.T Roles Settings and confirms access to coded app permission is selected for Partner role.', () => {
        cy.session(
            ...
            () => {
                ...
                // Locate and click edit option for Partner role.
                cy.get('a[title="View capabilities for Partner"]')
                    .should('be.visible')
                    .click()
    ...
    // The administrator goes to the Users page, locates the new test user  with the Partner role, and sets the password.
    it('The administrator goes to the Users page, locates the new test user  with the Partner role, and sets the password.', () => {
        cy.session(
            'the_administrator_goes_to_the_users_page_locates_the_new_test_user_with_the_partner_role_and_sets_the_password',
            () => {
                ...
                cy.get(
                    'input[name="dt_multi_role_user_roles[]"][value="partner"]'
                ).check()
    ...
    it('Log in to the home screen and verify that the configured coded app is visible and accessible for Partner.', () => {
        cy.session(
            ...
            () => {
                ...
                cy.get('dt-home-app-grid', { timeout: 10000 })
                    .shadow()
